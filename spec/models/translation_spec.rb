...
RSpec.describe Translation, type: :model do
  ...
  describe "validations" do
    ...
    context "when glossary_id points at a glossary that does not exist" do
      let(:translation) { build(:translation, glossary: nil, glossary_id: 999_999) }

      it "is invalid" do
        expect(translation).to be_invalid
        expect(translation.errors[:glossary]).to include("must exist")
      end

      it "is not persisted" do
        expect(translation.save).to be(false)
